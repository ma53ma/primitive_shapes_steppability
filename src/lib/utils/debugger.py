...
from grasp_kpts import BoxVertexType, TailVertexType, HedronVertexType

class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
               num_classes=-1, dataset=None, down_ratio=4, kpt_type=None):
    self.ipynb = ipynb
    if not self.ipynb:
    ...
    if dataset == 'coco_hp':
      ...
      self.colors_hp = [(255, 0, 255), (255, 0, 0), (0, 0, 255), 
        ...
        (255, 0, 0), (0, 0, 255), (255, 0, 0), (0, 0, 255),
        (255, 0, 0), (0, 0, 255)]
    elif dataset == "ps_grasp":
      self.names=["grasps"]
      self.num_class = 1
      self.num_kpts = 4
      self.kpt_type = kpt_type
      self.colors_hp = [0, 0, 0, 0]
      # bgr
      if self.kpt_type == "box":
        self.colors_hp[BoxVertexType.Left] = (125, 0, 0)
        self.colors_hp[BoxVertexType.Right] = (255, 0, 0)
        self.colors_hp[BoxVertexType.TopLeft] = (0, 0, 125)
        self.colors_hp[BoxVertexType.TopRight] = (0, 0, 255)
      elif self.kpt_type == "hedron":
        self.colors_hp[HedronVertexType.Left] = (125, 0, 0)
        self.colors_hp[HedronVertexType.Right] = (255, 0, 0)
        self.colors_hp[HedronVertexType.TopFront] = (0, 0, 125)
        self.colors_hp[HedronVertexType.TopBack] = (0, 0, 255)
      elif self.kpt_type == "tail":
        self.colors_hp[TailVertexType.Left] = (125, 0, 0)
        self.colors_hp[TailVertexType.Right] = (255, 0, 0)
        self.colors_hp[TailVertexType.Middle] = (0, 0, 125)
        self.colors_hp[TailVertexType.Tail] = (0, 0, 255)
      else:
  ...
  '''
  def gen_colormap_hp(self, img, output_res=None):
    """Generate the color map for the keypoint detection heatmap.
    The function takes in a pixel-wise probability prediction, 
    and generate the pixel-wise maximum weighted pre-defined keypoint-type color using the probability
    as the color map

    Args:
        img (array, (C, H, W)): The network detected kpt heatmap, where C is the keypoint type number
        output_res (_type_, optional): _description_. Defaults to None.

    Returns:
        color_map (H, W, 3):  The generated color map
    """
    c, h, w = img.shape[0], img.shape[1], img.shape[2]
    if output_res is None:
      output_res = (h * self.down_ratio, w * self.down_ratio)
  ...
  def add_ps_grasp_kpts(self, grasp_kpts, img_id='default'):
    """
    Args:
      grasp_kpts (array (N_kpt, 2)):    One set of the grasp keypoints
    """
    grasp_kpts = np.array(grasp_kpts, dtype=np.int32).reshape(1, self.num_kpts, 2)
    # function use rgb, where here they are bgr
    self.imgs[img_id] = plot_grasps_kpts(
      self.imgs[img_id][:,:,::-1], 
      grasp_kpts, 
      kpts_mode=self.kpt_type, 
      size=4
    )[:,:,::-1]
