...
class Base():
    ...
    Define the shared API name and functions
    """
    ...
    def __init__(self, color=np.random.choice(range(256), size=3).astype(np.uint8), pose=None,
        ...
        print('color assigned to face_colors: ', self.color)
    ...
    def get_grasp_meshes(self, gripper_frame=False):
        ...
        for i in range(self.grasp_poses.shape[0]):
            ...
            grasp = Grasp(open_width=open_width, pose=(self.pose @ T))
    ...
    def get_mesh(self, grasp=True, obj_frame=False, world_frame=False, gripper_frame=False):
        ...
        # grasp meshes
        if (grasp is True):
            mesh_list = mesh_list + self.get_grasp_meshes(gripper_frame=gripper_frame)
    ...
    def vis(self, grasp=True, obj_frame=False, world_frame=False, gripper_frame=False):
        """
        ...
        """
        # get the mesh
        mesh_list = self.get_mesh(grasp=grasp, obj_frame=obj_frame, world_frame=world_frame, gripper_frame=gripper_frame)
