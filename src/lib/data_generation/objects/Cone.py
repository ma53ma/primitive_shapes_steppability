...
class Cone(Base):
    ...
    """
    def __init__(self, radius, height, sections, color=np.random.choice(range(256), size=3).astype(np.uint8),
    ...
    def generate_mesh(self):
        obj_mesh = cone(radius=self.radius, height=self.height, sections=self.sections)
    ...
    def get_obj_dims(self):
        ...
        """
        return np.array([self.radius, self.height, self.sections])
