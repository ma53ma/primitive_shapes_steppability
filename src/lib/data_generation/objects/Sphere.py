...
class Sphere(Base):
    ...
    def _generate_grasp_semi(self, ang_sample_num=11, sink_in_dist=0.002):
        ...
        """
        grasp_poses = []
        open_width = 1.5 * (self.r - self.r_in)
...
"""The PSCNN code
def generate_sphere(port_num,r_input,sphere_flag=True):
...
# size=1
    scaleReduceFactor=100
    r=r_input/scaleReduceFactor
    # r = (size*0.25+2)/scaleReduceFactor
    if sphere_flag == True:
        phi, theta = np.mgrid[0:pi:101j, 0:2 * pi:101j]
    else:
    ...
    kp_3d=np.array([[-r,r,r],[r,r,r],[-r,-r,r],[r,-r,r],\
...
if __name__ == "__main__":
    sphere = Sphere(0.08, False)
    sphere.vis(True, False)

    semi_sphere = Sphere(0.08, True)
    semi_sphere.vis(True, False)