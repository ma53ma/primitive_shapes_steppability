...
class SceneRender(Scene):
    ...
    def __init__(self, table_size=1, table_thickness=0.04, table_color=[0.5, 0.5, 0.5, 1], \
        camera="realsense", camera_num=4, radius_range=[1, 1.2], latitude_range=[pi/6, pi/2]) -> None:
        ...
        Args:
            ...
            camera (str/4-by-4 array, optional): The camera name or the camera intrinsic. If set to a name then will use the pre-saved intrinsic.
    ...
    def render_imgs(self, instance_masks=False, grasp_mode=-1):
        ...
        for pose in self.camera_poses:
            ...
            if instance_masks:
                ins_masks_this_cam = self._render_steppability_masks(grasp_mode, pose, img_renderer)
                ins_masks.append(ins_masks_this_cam)
                #ins_masks_this_cam = self._render_instance_masks(scene, img_renderer, depth)
                #ins_masks.append(ins_masks_this_cam)

                # steppability_img, depth = img_renderer.render(scene, flags=flags)
    ...
    def _render_steppability_masks(self, grasp_mode, pose, img_renderer):
        ...
        flags = pyrender.RenderFlags.FLAT
        color, depth = img_renderer.render(scene, flags=flags)
