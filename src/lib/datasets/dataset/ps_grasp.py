...
class PSGrasp(data.Dataset):
    ...
    default_resolution = [512, 512]       # What does it do?

    # Mean and stddev of the RGB (or RGD if decided to use this)
    # NOTE: the mean and std adopted from the Cornell.
    # TODO: calculated that for the PSGrasp
    ...
    def __init__(self, opt, split):
        ...
        if self.opt.unitTest:
            ...
            #self.data_dir = os.path.join(opt.data_dir, "ps_grasp_single_10k")
            # self.data_dir = os.path.join(opt.data_dir, "ps_grasp_single_500")
            self.data_dir = os.path.join(opt.data_dir, "ps_grasp_single_1k")

        # color-jitter. NOTE: from the Cornell
        # TODO: do that for this dataset
    ...
    def _get_scene_info(self, scene_index):
        ...
        ##### NOTE: separate the grasp width from the projection width used for the pose estimation.
        #####       Hence not remap the width here
        # # remap the grasp width. Note that changing the width won't change the pose.
        # # NOTE: But collision condition might be changed in the multi-object scenerio
        # grasp_widths_new = []
        # if self.canonical_open_width is not None:
        #     for widths in grasp_widths:
        #         widths = np.ones_like(widths) * self.canonical_open_width
        #         grasp_widths_new.append(widths)
        # elif self.min_open_width is not None:
        #     grasp_widths_new = []
        #     for widths in grasp_widths:
        #         widths[widths < self.min_open_width] = self.min_open_width
        #         grasp_widths_new.append(widths)
        # else:
        #     grasp_widths_new = grasp_widths
        # grasp_widths = grasp_widths_new
    ...
    def run_eval(self, results, save_dir, **kwargs):
        ...
        for ind, (data_id, predicts) in enumerate(results.items()):
            ...
            for i in range(N_obj):
                ...
                # parse results
                metric_calculators["all"].update(pred_num, gt_num, pred_succ_num, gt_cover_num)
                metric_calculators[obj_types[i]].update(pred_num, gt_num, pred_succ_num, gt_cover_num)
        ...
        for key, val in metric_calculators.items():
            metric_calculators[key].print_results(key)
    ...
    def _get_gt_grasps(self, scene_idx, cam_idx, filter_collides=True, center_proj=False, trl_sample_num=-1, rot_sample_num=-1,
        ...
        Returns:
            poses_all (List[array], (N_obj, ) of (N_grasps, 4, 4)): The GT grasp poses. \\
    ...
    def _eval_grasps_with_gt(self, grasp_poses_pred, grasp_widths_pred, grasp_poses_gt_obj, grasp_widths_gt_obj,
        ...
        pred_succ_num = np.count_nonzero(pred_succ)
