...
def grasp_pose_decode(
        opt,
        heat, w, kps, reg=None, hm_kpts=None, kpts_offset=None, K=100):
    ...
    Returns:
        detections (tensor, [B, K', D]): Stack of the 2d keypoint projections, width, and the relevant info including:\
            ...
            i.e. detections = [center_location, kpt_locations, open_width, scores, clses]
            B is the batch size. K is the candidate number \
            D = 2 + 2*num_kpts + 1 + 1 + 1...
    ...
    if not opt.ori_clf_mode:
        centers, kps, w, scores, clses = _center_branch_decode(opt, heat, w, kps, reg, K)
    else:
        centers, kps, w, scores, clses = _center_branch_decode_ori(opt, heat, w, kps, reg, K)
...
def _center_branch_decode(opt, heat, w, kps, reg, K):
    """Get the top-K candidate keypoint sets from the center branch

    Args:
        heat (b, 1, h, w): Center heatmap. 
        w (b, 1, h, w): The open width 
        kps (): The keypoint center offset 
        reg (_type_): The center subpixel offset 
        K (int): top-k number
    ...
    """
    batch, cat, height, width = heat.size()
    num_kpts = kps.shape[1] // 2

    # perform nms on heatmaps
    if not opt.no_nms:
        heat = _nms(heat)

    # the top-k center point detection locations
    scores, inds, clses, ys, xs = _topk(heat, K=K)

    # the keypoint locations from the center point
    kps = _transpose_and_gather_feat(kps, inds)
    ...
    # get the open-width at the center point
    w = _transpose_and_gather_feat(w, inds)
    w = w.view(batch, K, 1)
    ...
    return centers, kps, w, scores, clses

def _center_branch_decode_ori(opt, heat, w, kps, reg, K):
    """Get the top-K candidate keypoint sets from the center branch
