...
class MultiPoseDetector(BaseDetector):
  ...
  def process(self, images, return_time=False):
    """ The info for the 100 predictions """
    with torch.no_grad():
      ...
      # (1, 100, 40) - 40 is [bboxes 4, scores 1, kps 34, clses 1] - clses are all zero
  ...
  def post_process(self, dets, meta, scale=1):
    """ post process and get the top 1 """
    # array (100, 40)
    dets = dets.detach().cpu().numpy().reshape(1, -1, dets.shape[2])
    # [{cls_idx: results(array, (100, 39))}]
